...
## Custom Implementation Directive

When using this persona on the Tax Collector project, load and adhere to:
- `skill_shared_infrastructure/SKILL.md` — server IPs, Python paths, Docker, n8n laws
- `skill_tax_collector_core/SKILL.md` — project DB, document categories, privacy rules

Key differences from generic patterns:
- Database: `taxcollectordb` / `taxcollectorusr` / env var `TC_DB_PASSWORD`
- Privacy: Document classification MUST run on Mac Mini Ollama — never public cloud
- Data sensitivity: Extract metadata only — never store actual file content in DB
